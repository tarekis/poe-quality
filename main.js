...
const calcCombinations = require('./lib/calcCombinations');
...
getItems().then(items => {
    ...
    if (gemCombinations.length > 0) {
        console.log('');
        console.log(`✔️  Vendor gems with those qualities for ${flaskCombinations.length} ` + 'Gemcutter\'s Prism'.rainbow + ':');
        console.log('   ' + gemCombinations.reduce(combinationReducer, []).sort(numericSorter).join(joinString));
    }

    if (mapCombinations.length > 0) {
        console.log('');
        console.log(`✔️  Vendor maps with those qualities for  ${flaskCombinations.length} ` + 'Cartographer\'s Chisel'.white + ':');
